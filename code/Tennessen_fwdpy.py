...
def make_parser():
    ...
    # genome options
    optional.add_argument('--selection_coeff', default=0.0, type=float, help="s from 2Ns")
    # Require 0 <= h <= 2
    ...
    optional.add_argument('--recombination_rate', '-r', default=2e-8, type=float)
...
def build_discrete_demography(args):
    ...
    T0 = np.rint((args.T_Af - args.T_B)/args.generation_time).astype(int) # pre-split
    T1 = np.rint((args.T_B - args.T_Eu_As)/args.generation_time).astype(int) # split to bottleneck, no growth
    T2 = np.rint((args.T_Eu_As - args.T_accel)/args.generation_time).astype(int) # Eu growth with r_Eu0
    T3 = np.rint(args.T_accel/args.generation_time).astype(int) # accelerated growth in Af and Eu
    ...
    size_change.append(fwdpy11.SetDemeSize(when=gens_burn_in, deme=0, new_size=args.NAfr0))
    ...
    size_change.append(fwdpy11.SetDemeSize(when=gens_burn_in+T0, deme=1, new_size=args.NB))
    # at the same time, set migration rate between deme 0 and 1 to m_A_B
    mig_rates.append(fwdpy11.SetMigrationRates(gens_burn_in+T0, 0, [1-args.mB, args.mB]))
    mig_rates.append(fwdpy11.SetMigrationRates(gens_burn_in+T0, 1, [args.mB, 1-args.mB]))
